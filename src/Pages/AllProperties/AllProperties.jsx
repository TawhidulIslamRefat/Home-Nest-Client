...
const AllProperties = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(
      `https://home-nest-server-psi.vercel.app/properties?search=${searchText}&sort=${sort}`
    )
      .then((res) => res.json())
        ...
        {properties.length === 0 ? (
          <>
            <div className=" w-[90%] sm:[400px] md:w-[600px] lg:w-[800px] flex flex-col justify-center items-center mx-auto">
              ...
              <Link to="/" className="btn bg-[#FF5A3C] text-white px-10 py-5">
                Go Back
              </Link>
            </div>
            <h1 className="text-sm md:text-2xl font-semibold text-center mb-20 mt-10 text-gray-600">
              No Properties Found.Try different search keyword
            </h1>
          </>
        ) : (
